...
class Hooks:
    ...
    def post_validations(req: Request, resp: Response, resource, params, model: Model = None, required_attributes: dict = None):
        # ids are not permitted in Post requests
        if "id" in params:
            resp.complete = True
            raise HTTPBadRequest(description="Id not valid in this request")
...
def validate_model_required_attributes(data, model):
    if model and isinstance(model, (set, list)):
        for item in model:
            validate_model_required_attributes(item)
        return
        ...
        if attr_type in (date, datetime):
            ...
            try:
                ...
                raise HTTPBadRequest(
                    description=f"Invalid attribute: {attr}. Expected a string with a valid date or datetime format"
                ) from err
